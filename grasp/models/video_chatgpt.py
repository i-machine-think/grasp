...
class VideoChatGPT(ModelWrapper):
    """Wrapper for Video-ChatGPT: https://github.com/mbzuai-oryx/Video-ChatGPT.
    """
    ...
    def __init__(self, config_file: str):
        ...
        model, vision_tower, tokenizer, image_processor, \
            video_token_len = initialize_model(
                self.config['model_path'], self.config['projection_path'])
    ...
    def reset(self):
        ...
        from video_chatgpt.video_conversation import (Conversation,
                                                      SeparatorStyle)
        self.state = Conversation(
            system=(
                'You are Video-ChatGPT, a large vision-language assistant. '
                'You are able to understand the video content that the '
                'user provides, and answer their questions correctly. '
                'Follow the instructions carefully and give short answers '
    ...
    def submit(self, prompt: str, video: str = None, verbose: bool = True):
        ...
        gen = self.chat.answer(self.state, self.img_list,
                               self.config['temperature'],
                               self.config['max_output_tokens'], False)
